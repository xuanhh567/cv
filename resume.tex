...
\begin{minipage}[c]{0.82\textwidth}
    ...
    华中科技大学 \quad $\cdot$ \quad 电子信息与通信学院 \\[0.2em]
    本科生 \quad $\cdot$ \quad 预计 2027 年 6 月毕业 \\[0.2em]
...
{\normalsize \textbf{研究方向：机器人、计算机视觉、具身智能与视觉-语言-动作模型}}
...
\section{论文发表与学术会议}
\begin{itemize}
    \item Yefan Lin, \textbf{Jiaxuan Wang}, Baoyu Liu, Zheng Lv, and Xiaojun Hei, ``Pallet-ACT: A Unified Data-Training-Inference Framework for Industrial Robotic Palletizing.'' \textit{7th Information Communication Technologies Conference (\textbf{ICTC})}, Nanjing, China, May 2026. (\textbf{已录用})
...
\section{技能、工具与领导力}
\begin{itemize}
    \item \textbf{编程与具身智能算法}：熟练掌握 Python 与 C 语言；掌握强化学习（RL）、模仿学习（IL）等算法在机器人控制任务中的应用；成功复现 Pi 系列 VLA 模型，具备阅读前沿模型代码并完成实验复现的能力。
    \item \textbf{机器人控制与真机部署}：具备丰富的 6 轴机械臂真机实操、调试与部署经验；熟悉 ROS 2 机器人操作系统及 MoveIt 2 运动规划框架，能够完成从仿真验证到真实系统部署的工程闭环。
    \item \textbf{仿真、建模与工程实现}：熟练应用 MuJoCo 物理引擎进行机器人仿真与测试；精通 SolidWorks 三维建模与产品结构设计，并具备丰富的 3D 打印实际操作经验。
    \item \textbf{团队领导与项目推进}：现任 Dian 团队队长及机器人组组长，长期组织团队任务分工、技术路线推进、项目展示与竞赛协作，具备较强的沟通协调、执行落地与抗压能力。
    \item \textbf{科研展示与技术传播}：长期关注摄影、视频制作与技术展示，能够为机器人项目制作演示视频、实验记录和技术说明材料，支持科研成果展示、项目汇报与学术传播。
    \item \textbf{开发环境与工具}：熟悉 Linux、VSCode、VMware、Anaconda 等开发与环境管理工具；善于运用现代 AI 工具辅助编程、文档编写与科研工作；保持游泳、篮球、羽毛球等运动习惯。
\end{itemize}
...
\vspace{0.2em}
\textbf{基于模仿学习的化学实验机械臂智能应用设计} | \textbf{省级大创项目 (队长，结题优秀)} \hfill 2025年度
\begin{itemize}
    \item 作为项目队长，负责整体推进、任务分工与技术落地，完成机械臂对化学实验器材的精准抓取。(指导教师：黑晓军)
\end{itemize}
